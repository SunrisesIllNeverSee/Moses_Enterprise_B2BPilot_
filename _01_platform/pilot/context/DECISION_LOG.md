...
| 2026-09-06 | Deep-dive review placed in review/ as lead decision memo | (a) In context/ as design history; (b) In pilot/ root | The deep-dive review (`UPSILON_ORIGINAL_TO_CURRENT_DEEP_DIVE_REVIEW.md`) is a review memo that preceded and guided the canonization. It complements `IMPLEMENTATION_REVIEW_MEMO.md` (which reviews the implementation). `review/` is the natural home for review memos. | ACCEPTED | — |
| 2026-09-06 | Original dossier traceability, drift analysis, and reporting invariants added to context/ | (a) In implementation/ as gaps; (b) In governance/ as canonical | These three documents (`ORIGINAL_DOSSIER_TRACEABILITY.md`, `DRIFT_AND_CONTRADICTIONS.md`, `REPORTING_INTEGRITY_INVARIANTS.md`) are analytical/recommendation documents that feed canon and implementation — they are not themselves canonical. Placing them in context/ preserves the reasoning without freezing it. The reporting invariants that are PROPOSED (not yet canonical) require promotion before becoming normative. | ACCEPTED | — |
